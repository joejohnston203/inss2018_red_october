...
class AnalyzeAntiNueCounts:
    ...
    def __init__(self, ndim, t, tMax, expBackground, detVolFactor):
        """
        ...
        self.ndim = ndim
        self.x = x
        self.expBackground = expBackground
    ...
    def __call__(self, params):
        ...
        x_model = np.linspace(0, tMax, 500)
        y_model = np.array([rate(params, t) for t in x_model])
    ...
    def rate(self, params, t):
        ...
        expRate = expBackround + (detVolFactor*baseRate*subPower/basePower*baseDistance**2) * subDist
    ...
    def log_lh(self, params):
        ...
        NexpTot = integrate.trapz(y_model, x_model)
        L = poisson.pmf(len(self.t), NexpTot)
        for i in range(len(self.t)):
            L = L * self.rate(self.t[i])
    ...
    def show_results(self, burnin):
        ...
        sns.distplot(self.t, c='k', hist=False, label='Data KDE')
